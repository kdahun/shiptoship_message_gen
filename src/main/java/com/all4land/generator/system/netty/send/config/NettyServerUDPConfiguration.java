...
import com.all4land.generator.system.util.NetWorkUtil;
...
public class NettyServerUDPConfiguration implements AutoCloseable {
	...
	public Bootstrap udpServerBootstrap(NettyUdpServerConfig nettyUdpServerConfig) {
		...
		this.multicastInfo = new InetSocketAddress(multiCastInfo[0], Integer.parseInt(multiCastInfo[1]));
...
//		this.udpBootstrap.option(ChannelOption.IP_MULTICAST_TTL, 255);
		...
		try {
			NioDatagramChannel ch = (NioDatagramChannel) this.udpBootstrap.bind(Integer.parseInt(multiCastInfo[1])).sync().channel();
